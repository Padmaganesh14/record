...
var Context = import_react3.default.createContext({
  activeIndex: -1,
...
function SortableContext(_ref) {
  ...
  const items = (0, import_react3.useMemo)(() => userDefinedItems.map((item) => typeof item === "object" && "id" in item ? item.id : item), [userDefinedItems]);
  const isDragging = active != null;
  const activeIndex = active ? items.indexOf(active.id) : -1;
  const overIndex = over ? items.indexOf(over.id) : -1;
  const previousItemsRef = (0, import_react3.useRef)(items);
...
function useDerivedTransform(_ref) {
  ...
  const [derivedTransform, setDerivedtransform] = (0, import_react3.useState)(null);
  const previousIndex = (0, import_react3.useRef)(index);
  useIsomorphicLayoutEffect(() => {
...
function useSortable(_ref) {
  ...
  const itemsAfterCurrentSortable = (0, import_react3.useMemo)(() => items.slice(items.indexOf(id)), [items, id]);
  const {
    rect,
    node,
    isOver,
